...
yearly_forecast_plot <- function(cur_m_power, new_m_power, new_m_price, el_cost, cur_month_power) {
  ...
  new_month_power <- new_m_power*(cur_month_power$kWh/cur_m_power)
  plot_vector <- yearly_data_to_plot(cur_m_power, new_m_power, new_m_price, el_cost, cur_month_power, new_month_power)
  plot_data <- data.frame(plot_vector$time, plot_vector$variable, plot_vector$value)
  colnames(plot_data) <- c("time", "variable", "value")
  meet_cost <- plot_vector$meet_cost
  end_date <- plot_vector$end_date
  fut_date <- plot_vector$fut_date
  years_to_go <- plot_vector$years_to_go
...
#' 
#' @return list (dataframe (for plot), meetcost, end_date, fut_date, years_to_go)
#' 
#' @export
#'
#' @import lubridate
...
yearly_data_to_plot <- function(cur_m_power, new_m_power, new_m_price, el_cost, cur_month_power, new_month_power) {
  ...
  plot_data <- tidyr::pivot_longer(data.frame(time, cur_m_cost, new_m_cost),
                                   ...
                                   names_to = "variable", 
                                   values_to = "value")
  return(list("time" = plot_data$time, "variable" = plot_data$variable, "value" = plot_data$value, "meet_cost" = meet_cost,
              "end_date" = end_date, "fut_date" = fut_date, "years_to_go" = years_to_go))
}
